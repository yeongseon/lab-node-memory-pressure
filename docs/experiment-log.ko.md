...
*그림 7: 정상 상태 트래픽 볼륨(RPM)과 CPU% 오버레이. 모든 단계에서 트래픽은 ~6 RPM(앱당 10초에 1건 × 6개 앱)으로 일정하게 유지되었으나, Memory Pressure 시 CPU는 15%에서 35% 이상으로 상승 — CPU 증가가 트래픽에 의한 것이 아님을 확인.*
...
*그림 12: 컨테이너 배포의 정상 상태 트래픽 볼륨(RPM) vs CPU%. Memory Pressure 단계 동안 트래픽은 일관되게 낮게 유지되었으나 CPU는 변동했습니다.*
