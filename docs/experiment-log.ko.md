...
*그림 1: App Service Plan CPU%(주황색)와 Memory%(빨간색)를 0-100% 공유 스케일로 표시. 앱 스케일업에 따라 두 메트릭이 함께 상승하며, CPU는 ~15%에서 47%로, Memory는 92%까지 도달.*

![Kernel Reclaim Activity](assets/charts/zip-deploy/kernel-reclaim-timeline.png)
*그림 2: Linux 커널 페이지 reclaim 카운터(구간당 Δ). pgscan_kswapd 및 pgscan_direct의 급격한 증가는 Memory Pressure 하에서 커널이 적극적으로 페이지를 회수하고 있음을 확인.*

![Swap Activity](assets/charts/zip-deploy/swap-activity-timeline.png)
*그림 3: Swap I/O 활동(구간당 Δ). 격렬한 pswpin/pswpout 스파이크가 높은 메모리 단계와 상관관계를 보이며, 커널이 RAM과 디스크 사이에서 페이지를 이동하는 swap thrashing을 확인.*

![Memory Breakdown](assets/charts/zip-deploy/memory-breakdown-timeline.png)
*그림 4: /proc/meminfo 기반 OS 수준 메모리 분석. 앱 메모리 할당이 증가함에 따라 MemFree, MemAvailable, Cached, SwapFree가 감소하는 모습.*

![App RSS](assets/charts/zip-deploy/app-rss-timeline.png)
*그림 5: 앱별 RSS(Resident Set Size, MB). 각 앱의 메모리 사용량이 단계별 ALLOC_MB 증가에 따라 성장.*

![Burst Latency Distribution](assets/charts/zip-deploy/burst-latency-distribution.png)
*그림 6: Phase 3 트래픽 버스트(10 RPS × 60초) 중 Latency 히스토그램. Memory Pressure 하에서의 요청 지연 분포를 표시.*
...
*그림 8: 컨테이너 배포의 App Service Plan CPU%(주황색)와 Memory%(빨간색)를 0-100% 공유 스케일로 표시. Memory는 ~75-82%를 유지하는 반면, CPU는 컨테이너 스케일링 이벤트(Phase 2a) 중 84%까지 급등 후 10-24%로 안정화.*

![Kernel Reclaim Activity](assets/charts/container-deploy/kernel-reclaim-timeline.png)
*그림 9: 컨테이너 환경의 커널 페이지 reclaim 활동. pgscan_kswapd 및 pgscan_direct 카운터가 ZIP 배포 대비 완만한 reclaim을 보임.*

![Swap Activity](assets/charts/container-deploy/swap-activity-timeline.png)
*그림 10: 컨테이너 실험 중 Swap I/O. 컨테이너 격리로 swap 사용률이 ~49%로 제한되어 ZIP 배포보다 낮은 강도.*

![Burst Latency Distribution](assets/charts/container-deploy/burst-latency-distribution.png)
*그림 11: 컨테이너 Phase 3 버스트 Latency 히스토그램. ZIP 대비 높은 분산(평균 174ms vs 17ms)으로 컨테이너가 Memory Pressure에 더 민감함을 확인.*
